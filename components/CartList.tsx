    ...
    (newQuantity: number) => {
      if (newQuantity < 0 || newQuantity > item.stock) {
        toast({
          ...
          description: `Quantity must be between 0 and ${item.stock}`,
          variant: "destructive",
        });
        return;
      }
      ...
      startTransition(async () => {
        try {
          ...
          if (result?.error) {
            ...
            onUpdate(item.productId, previousQuantity);
            toast({
              title: "Error",
            ...
            result?.updatedQuantity !== undefined &&
            result.updatedQuantity !== newQuantity
          ) {
            // Server returned a different quantity (e.g., due to stock changes)
            setQuantity(result.updatedQuantity);
            onUpdate(item.productId, result.updatedQuantity);
            toast({
              title: "Notice",
              description: "Quantity adjusted due to stock availability",
            });
          } else {
            toast({
              title: "Success",
