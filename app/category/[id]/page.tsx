...
export default async function CategoryPage({
  ...
  const products: Product[] = await Promise.all(
    (
      ...
      category: prod.category_id
        ? (await getCategory(prod.category_id)) || undefined
        : undefined,
    }))
  );
  ...
  const favorites: string[] | null =
    user?.favorites?.map((fav) => fav.productId || "").filter(Boolean) || null;
...
export async function generateMetadata({
  ...
  if (!category) return { title: "Category Not Found | Aldhabi Store" };
  const title = `${category.name} | Aldhabi Store`;
  const description =
    category.description || `Shop ${category.name} at Aldhabi Store.`;
